...
import { Clock, IndianRupee, Save, AlertCircle, CheckCircle, MapPin, Plus, Trash2, Search, X, Gift, Calendar, Percent, Users } from 'lucide-react';
...
const Settings = () => {
    ...
    const fetchCoupons = async () => {
        try {
            const couponsSnapshot = await getDocs(collection(fireDB, 'coupons'));
    ...
    const handleAddCoupon = async () => {
        if (!newCoupon.code.trim() || !newCoupon.value || !newCoupon.validFrom || !newCoupon.validUntil) {
            toast.error('Please fill all required fields');
            return;
        }

        if (newCoupon.type === 'percentage' && (Number(newCoupon.value) <= 0 || Number(newCoupon.value) > 100)) {
            toast.error('Percentage must be between 1 and 100');
            return;
        }

        if (newCoupon.type === 'fixed' && Number(newCoupon.value) <= 0) {
        ...
        try {
            ...
            await addDoc(collection(fireDB, 'coupons'), couponData);
    ...
    const handleToggleCouponStatus = async (couponId, currentStatus) => {
        try {
            await setDoc(doc(fireDB, 'coupons', couponId), {
                ...
                <div className="bg-gradient-to-r from-purple-50 to-pink-50 px-6 py-4 border-b border-gray-100">
                    <div className="flex items-center justify-between">
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 bg-purple-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-purple-700 transition-all"
                        >
                            <Plus className="w-4 h-4" />
                ...
                <div className="p-6">
                        ...
                        <div className="mb-6 p-6 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg border border-purple-200">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        className="w-full border-2 border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-purple-200 focus:border-purple-400 transition-all outline-none"
                                ...
                                <div className="md:col-span-2 lg:col-span-1">
                                    ...
                                    <input
                                        type="text"
                                        className="w-full border-2 border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-purple-200 focus:border-purple-400 transition-all outline-none"
                                        placeholder="e.g., 20% off on all vegetables"
                            ...
                            <div className="mt-4 flex justify-end">
                                <button
                                    ...
                                    className="flex items-center gap-2 bg-purple-600 text-white px-6 py-2 rounded-lg font-semibold hover:bg-purple-700 transition-all disabled:opacity-50"
                                    disabled={couponLoading || !newCoupon.code.trim() || !newCoupon.value || !newCoupon.validFrom || !newCoupon.validUntil}
                                >
                                    <Gift className="w-4 h-4" />
                    ...
                    <div className="mb-4">
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                className="w-full pl-10 pr-4 py-2 border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-200 focus:border-purple-400 transition-all outline-none"
                    ...
                    <div className="space-y-3 max-h-96 overflow-y-auto">
                        {filteredCoupons.length > 0 ? (
                            filteredCoupons.map((coupon) => {
                                ...
                                return (
                                    <div key={coupon.id} className={`p-4 rounded-lg border transition-all ${
                                        ...
                                        <div className="flex items-start justify-between">
                                            <div className="flex-1">
                                                <div className="flex items-center gap-3 mb-2">
                                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                                                        ...
                                                        <Gift className={`w-4 h-4 ${isActive ? 'text-green-600' : 'text-gray-500'}`} />
                                                    </div>
                                                    <div>
                                                        <div className="flex items-center gap-2">
                                                            <span className="font-bold text-lg text-purple-700">{coupon.code}</span>
                                            ...
                                            <div className="flex flex-col gap-2 ml-4">
                                                ...
                                                <button
                                                    ...
                                                    className="bg-red-500 text-white px-3 py-1 rounded text-sm font-semibold hover:bg-red-600 transition-all"
                                                >
                                                    Delete
                                                </button>
                                            </div>
                            ...
                            <div className="text-center py-8 text-gray-500">
                                {couponSearch ? 'No coupons found matching your search' : 'No coupons created yet'}
                            </div>
                        )}
                    </div>
                </div>
